...
use ActiveRecord\db\ActiveQueryInterface;
use ActiveRecord\db\ActiveRecordInterface;
...
class UniqueValidator extends Validator
{
    ...
    public $targetClass;
    /**
     * @var string|array the name of the [[\ActiveRecord\db\ActiveRecord|ActiveRecord]] attribute that should be used to
     ...
     * This can be a string or an array representing the additional query condition (refer to [[\ActiveRecord\db\Query::where()]]
     * on the format of query condition), or an anonymous function with the signature `function ($query)`, where `$query`
     * is the [[\ActiveRecord\db\Query|Query]] object that you can modify in the function.
     */
    public $filter;
    /**
    ...
    public function init()
    {
        ...
        if (is_array($this->targetAttribute) && count($this->targetAttribute) > 1) {
            ...
            if ($this->comboNotUnique === null) {
                $this->message = 'The combination {values} of {attributes} has already been taken.';
            } else {
     ...
     * @param array $conditions conditions, compatible with [[\ActiveRecord\db\Query::where()|Query::where()]] key-value format.
     * @param Model $model the data model to be validated
     *
    ...
    private function modelExists($targetClass, $conditions, $model)
    {
        ...
        if (!$model instanceof ActiveRecordInterface || $model->getIsNewRecord() || $model->className() !== $targetClass::className()) {
            ...
            if ($query instanceof \ActiveRecord\db\ActiveQuery) {
     ...
     * [[\ActiveRecord\db\Query::where()|Query::where()]] key-value format.
     *
     * @param string|array $targetAttribute the name of the [[\ActiveRecord\db\ActiveRecord|ActiveRecord]] attribute that
     ...
     *
     * @return array conditions, compatible with [[\ActiveRecord\db\Query::where()|Query::where()]] key-value format.
     */
     ...
     * @param \ActiveRecord\base\Model $model the data model.
