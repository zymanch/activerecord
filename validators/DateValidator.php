...
use ActiveRecord\i18n\Formatter;
...
class DateValidator extends Validator
{
     ...
     *
     * If this property is not set, the default value will be obtained from `ActiveRecord::$app->formatter->dateFormat`, see [[\ActiveRecord\i18n\Formatter::dateFormat]] for details.
     ...
     *
     * - if type is [[TYPE_DATE]], the default value will be taken from [[\ActiveRecord\i18n\Formatter::dateFormat]],
     * - if type is [[TYPE_DATETIME]], it will be taken from [[\ActiveRecord\i18n\Formatter::datetimeFormat]],
     * - and if type is [[TYPE_TIME]], it will be [[\ActiveRecord\i18n\Formatter::timeFormat]].
     *
     ...
     * This is only effective when the [PHP intl extension](http://php.net/manual/en/book.intl.php) is installed.
     * If not set, the locale of the [[\ActiveRecord\base\Application::formatter|formatter]] will be used.
     * See also [[\ActiveRecord\i18n\Formatter::locale]].
     */
    public $locale;
    /**
    ...
    public function init()
    {
        ...
        if ($this->format === null) {
            ...
            if ($this->type === self::TYPE_DATE) {
                $this->format = $formatter->dateFormat;
            } elseif ($this->type === self::TYPE_DATETIME) {
                $this->format = $formatter->datetimeFormat;
            } elseif ($this->type === self::TYPE_TIME) {
                $this->format = $formatter->timeFormat;
            } else {
